...
contract BuilderNFT is ERC1155, Ownable {
    ...
    function mintBuilderNft(uint256 tokenId, uint256 amount, string calldata scout) external {
      ...
      // 1. Validate scout ID
      require(isValidUUID(scout));

      // 2. Make sure the builder id is valid
      ...
      // 3. Require non zero amount of tokens purchased
      require(amount > 0, "Must buy at least one token");

      // 4. Get cost to buy
      uint256 mintCost = getTokenPurchasePrice(tokenId, amount);
    ...
    function getTokenURI(uint256 tokenId) public view returns (string memory) {
        return string(abi.encodePacked(baseUrl, uintToString(tokenId), "/image.jpeg"));
    }
}