...
describe('ScoutTokenERC20', function () {
  ...
  beforeEach(async () => {
    ...
    erc20AdminAccount = token.ProtocolERC20AdminAccount;
  ...
  describe('transfer', function () {
    describe('effects', function () {
      it('transfers tokens correctly', async function () {
        ...
        const adminBalanceBeforeTransfer = await token.ProtocolERC20.read.balanceOf([
        ...
        await token.ProtocolERC20.write.transfer([userAccount.account.address, firstTransfer], {
          account: erc20AdminAccount.account
        });

        await token.ProtocolERC20.write.transfer([userAccount.account.address, secondTransfer], {
          account: erc20AdminAccount.account
        });

        const userBalance = await token.ProtocolERC20.read.balanceOf([userAccount.account.address]);
        expect(userBalance).toEqual(totalTransfer);

        const adminBalance = await token.ProtocolERC20.read.balanceOf([erc20AdminAccount.account.address]);
    ...
    describe('events', function () {
      it('emits Transfer event on transfer', async function () {
        ...
        // Transfer tokens from admin to user
        const txResponse = await token.ProtocolERC20.write.transfer([userAccount.account.address, BigInt(500)], {
          account: erc20AdminAccount.account
        });
    ...
    describe('permissions', function () {
      it('allows token holders to transfer their tokens', async function () {
        ...
        await expect(
          token.ProtocolERC20.write.transfer(
            [secondUserAccount.account.address, BigInt(500) * token.ProtocolERC20_DECIMAL_MULTIPLIER],
            {
        ...
        const secondUserBalance = await token.ProtocolERC20.read.balanceOf([secondUserAccount.account.address]);
        expect(secondUserBalance).toEqual(BigInt(500) * token.ProtocolERC20_DECIMAL_MULTIPLIER);
      });
    });
    ...
    describe('validations', function () {
      it('prevents transferring more tokens than balance', async function () {
        ...
        await expect(
          token.ProtocolERC20.write.transfer(
            [secondUserAccount.account.address, BigInt(2000) * token.ProtocolERC20_DECIMAL_MULTIPLIER],
            {
              account: userAccount.account
            }
          )
  ...
  describe('approve', function () {
    describe('effects', function () {
      it('approves allowance correctly', async function () {
        await expect(
          token.ProtocolERC20.write.approve([secondUserAccount.account.address, BigInt(1000)], {
            account: userAccount.account
          })
        ).resolves.toBeDefined();

        const allowance = await token.ProtocolERC20.read.allowance([
    ...
    describe('events', function () {
      it('emits Approval event on approve', async function () {
        const txResponse = await token.ProtocolERC20.write.approve([secondUserAccount.account.address, BigInt(1000)], {
          account: userAccount.account
        });
  ...
  describe('transferFrom', function () {
    describe('effects', function () {
      it('transfers tokens correctly using allowance', async function () {
        ...
        // Approve second user
        await token.ProtocolERC20.write.approve(
          [secondUserAccount.account.address, BigInt(500) * token.ProtocolERC20_DECIMAL_MULTIPLIER],
          {
            account: userAccount.account
          }
        ...
        await token.ProtocolERC20.write.transferFrom(
          [
            userAccount.account.address,
            thirdAccount.account.address,
            BigInt(500) * token.ProtocolERC20_DECIMAL_MULTIPLIER
          ],
          { account: secondUserAccount.account }
        );

        const userBalance = await token.ProtocolERC20.read.balanceOf([userAccount.account.address]);
        expect(userBalance).toEqual(BigInt(500) * token.ProtocolERC20_DECIMAL_MULTIPLIER);

        const recipientBalance = await token.ProtocolERC20.read.balanceOf([thirdAccount.account.address]);
        expect(recipientBalance).toEqual(BigInt(500) * token.ProtocolERC20_DECIMAL_MULTIPLIER);
      });
    });
    ...
    describe('events', function () {
      it('emits Transfer event on transferFrom', async function () {
        ...
        // Approve second user
        await token.ProtocolERC20.write.approve([secondUserAccount.account.address, BigInt(500)], {
          account: userAccount.account
        });

        // Transfer from user to admin using second user's account
        const txResponse = await token.ProtocolERC20.write.transferFrom(
    ...
    describe('permissions', function () {
      it('allows spender to transfer within allowance', async function () {
        ...
        await expect(
          token.ProtocolERC20.write.transferFrom(
            [userAccount.account.address, erc20AdminAccount.account.address, BigInt(500)],
            { account: secondUserAccount.account }
          )
        ).resolves.toBeDefined();
  ...
  describe('increaseAllowance', function () {
    describe('effects', function () {
      it('increases allowance correctly', async function () {
        await token.ProtocolERC20.write.approve([secondUserAccount.account.address, BigInt(1000)], {
          account: userAccount.account
        });

        await token.ProtocolERC20.write.increaseAllowance([secondUserAccount.account.address, BigInt(500)], {
          account: userAccount.account
        });
    ...
    describe('events', function () {
      it('emits Approval event on increaseAllowance', async function () {
        const txResponse = await token.ProtocolERC20.write.increaseAllowance(
  ...
  describe('decreaseAllowance', function () {
    describe('effects', function () {
      it('decreases allowance correctly', async function () {
        ...
        await token.ProtocolERC20.write.decreaseAllowance([secondUserAccount.account.address, BigInt(500)], {
          account: userAccount.account
        });
    ...
    describe('validations', function () {
      it('reverts when decreasing allowance below zero', async function () {
        ...
        await expect(
          token.ProtocolERC20.write.decreaseAllowance([secondUserAccount.account.address, BigInt(600)], {
            account: userAccount.account
          })
