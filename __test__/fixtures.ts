...
async function deployProtocolFixtures() {
  ...
  const protocol = await deployProtocolContract({ ScoutTokenERC20Address: token.ScoutTokenERC20Implementation.address });
...
async function deployScoutProtocolBuilderNFTFixtures() {
  ...
  const scoutProtocolBuilderNft = await deployScoutProtocolBuilderNftContract({
    ScoutScoutTokenERC20Address: token.ScoutTokenERC20Implementation.address
  });
