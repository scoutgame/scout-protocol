...
describe('deployUsdc', () => {
  ...
  it('should deploy the USDC contract, with minting, approve, transfer, balanceOf and transferFrom enabled', async () => {
    ...
    await approveUSDC({
      args: { spender: secondUserAccount.account.address, amount: hundredUsdc },
      wallet: userAccount
    });

    // test the transferFrom function
    await transferUSDCFrom({
      args: { from: userAccount.account.address, to: secondUserAccount.account.address, amount: hundredUsdc },
