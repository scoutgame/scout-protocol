...
task('deployEASSchemas', 'Deploys or updates the BuilderNFTSeasonOne contracts').setAction(async (taskArgs, hre) => {
  ...
  const deployedResolver = await viem.deployContract(
    'ProtocolEASResolver',
    [connector.easAttestationContract as Address, account.address],
    {
      client: {
        wallet: walletClient
      }
  ...
  try {
    execSync(`npx hardhat verify --network ${getConnectorKey(connector.chain.id)} ${resolverAddress}`);
  } catch (err) {
